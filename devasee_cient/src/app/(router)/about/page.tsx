    ...
    useLayoutEffect(() => {
        const ctx = gsap.context(() => {
            ...
            const createBubbles = () => {
                ...
                // Create bubbles - reduced count for performance
                const bubbleCount = 12;
                for (let i = 0; i < bubbleCount; i++) {
                    const bubble = document.createElement('div');
                    bubble.className = 'absolute rounded-full bg-blue-500/30';

                    // Smaller sizes for faster movement
                    ...
                    // Faster animation with more movement
                    const duration = Math.random() * 4 + 3; // 3-7 seconds
                    const distance = Math.random() * 200 + 100; // 100-300px

                    // Animate bubble - faster movement with rotation and scaling
                    gsap.to(bubble, {
                ...
                <section className="gallery-container h-screen flex items-center overflow-hidden relative z-10">
                    <div className="gallery-wrapper flex gap-6 md:gap-8 py-8 pl-8">
                        ...
                        {galleryImages.map((image, index) => (
                            <div
                                key={index}
                                className="gallery-item rounded-xl shadow-md hover:shadow-2xl transition-all duration-300 overflow-hidden flex-shrink-0
                         w-[70vw] xs:w-[60vw] sm:w-[50vw] md:w-[400px] h-[50vh] md:h-[55vh]"
                            >
                                <Image
                                    src={image.src}
                                    alt={image.alt}
