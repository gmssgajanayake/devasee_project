...
export default function MainNavBar() {
    ...
    return (
        <div className="w-screen h-auto  bg-white relative">
            ...
            <nav
                ref={navRef}
                className={`w-full fixed top-9 z-50 transition-all duration-500 px-5 lg:px-8 py-3 flex items-center justify-between ${isScrolled ? "bg-white/70 backdrop-blur-md shadow-md" : "bg-white"}`}
            >
                <Link href="/">
                    <Image
                        src={logo}
                ...
                <div className="flex lg:hidden items-center">
                    <AlignJustify size={28} className={"text-gray-600 cursor-pointer "+(isMobileMenuOpen ? 'hidden' : 'block')} onClick={toggleMobileMenu} />
                    <X size={28} className={"text-gray-600 cursor-pointer ml-4 " + (isMobileMenuOpen ? 'block' : 'hidden')} onClick={toggleMobileMenu} />
                </div>
            </nav>
            {/* Mobile menu bar */}
            <div
                ref={menuRef}
                className={
                    "lg:hidden absolute bg-white/30 backdrop-blur-md inset-x-0 top-24 z-50 " +
